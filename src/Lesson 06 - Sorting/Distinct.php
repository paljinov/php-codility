 ...
 * Distinct task.
 *
 * CODILITY ANALYSIS: https://codility.com/demo/results/trainingRXXFGB-DJM/
 * LEVEL: EASY
 * Correctness: 100%
 * Performance: 100%
 * Task score:  100%
 ...
 *
 * @return int The number of distinct values in array A
 */
function solution($A)
{
    ...
    // Array which holds distinct values from array $A
    $distinct = [];

    foreach ($A as $integer) {
        // Saving distinct values by key ensures uniqueness
        if (!isset($distinct[$integer])) {
            $distinct[$integer] = $integer;
        }
    }

    // The number of distinct values in array $A
